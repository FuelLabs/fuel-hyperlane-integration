...
pub enum MultisigIsmError {
    ...
    DomainAndValidatorLengthMismatch: (u64, u64),
    DomainAndThresholdLengthMismatch: (u64, u64),
    ValidatorNotEnrolled: (u32, EvmAddress),
}
...
impl InterchainSecurityModule for Contract {
    ...
    fn verify(metadata: Bytes, message: Bytes) -> bool {
        ...
        let threshold = _threshold(message.origin());
        let metadata = MultisigMetadata::from_bytes(metadata, u64::from(threshold));
        require(
            _verify_merkle_proof(metadata, message),
            MultisigIsmError::FailedToVerifyProof,
        );
        require(
            _verify_validator_signatures(u64::from(threshold), metadata, message),
            MultisigIsmError::FailedToVerifySignatures,
        );
        return true;
    }
}
...
impl MultisigIsm for Contract {
    ...
    fn set_threshold(domain: u32, threshold: u8) {
        only_owner();
        _set_threshold(domain, threshold);
    }
    ...
    fn enroll_validator(domain: u32, validator: EvmAddress) {
        only_owner();
        _enroll_validator(domain, validator);
    }
    ...
    fn enroll_validators(domains: Vec<u32>, validators: Vec<Vec<EvmAddress>>) {
        ...
        require(
            domain_len == validators_len,
            MultisigIsmError::DomainAndValidatorLengthMismatch((domain_len, validators_len)),
        );
    ...
    fn set_thresholds(domains: Vec<u32>, thresholds: Vec<u8>) {
        only_owner();
        let domain_len = domains.len();
        let thresholds_len = thresholds.len();
        require(
            domain_len == thresholds_len,
            MultisigIsmError::DomainAndThresholdLengthMismatch((domain_len, thresholds_len)),
        );

        let mut i = 0;
        while i < domain_len {
            _set_threshold(domains.get(i).unwrap(), thresholds.get(i).unwrap());
            i += 1;
        }
    }
    ...
    fn unenroll_validator(domain: u32, validator: EvmAddress) {
        only_owner();
        let index = _index_of(domain, validator);
        require(
            index
                .is_some(),
            MultisigIsmError::ValidatorNotEnrolled((domain, validator)),
        );
...
#[storage(read)]
fn _index_of(domain: u32, validator: EvmAddress) -> Option<u64> {
...
#[storage(read)]
fn _is_enrolled(domain: u32, validator: EvmAddress) -> bool {
    let validators = storage.validators.get(u64::from(domain));
    // if validators.is_some() {
    //     let len = validators.unwrap().len();
    //     return _index_of()
    // } // TODO
    return _index_of(domain, validator).is_some();
}
...
pub fn _verify_merkle_proof(metadata: MultisigMetadata, message: EncodedMessage) -> bool {
...
#[storage(read)]
pub fn _verify_validator_signatures(
    threshold: u64,
...
#[storage(read, write)]
fn _enroll_validator(domain: u32, validator: EvmAddress) {
    require(validator != EvmAddress::from(ZERO_B256), "zero address");
    require(!_is_enrolled(domain, validator), "enrolled");
...
#[storage(read, write)]
fn _set_threshold(domain: u32, threshold: u8) {
    let domain_u64 = u64::from(domain);
    let validators = storage.validators.get(domain_u64);
...
#[storage(read)]
fn _threshold(domain: u32) -> u8 {
    storage.threshold.get(domain).try_read().unwrap_or(0)
}

// Returns the validator set enrolled for the domain.
#[storage(read)]
fn _validators(domain: u32) -> Vec<EvmAddress> {
    let validators = storage.validators.get(u64::from(domain));
    if validators.is_none() {
        return Vec::new();
    }
