use crate::contracts::sepolia_mailbox::SepoliaMailbox::SepoliaMailboxInstance;
use crate::helper::*;
use std::env;
...
impl Contracts {
    ...
    pub async fn fuel_send_dispatch(&self, with_igp: bool) {
        let recipient_address = hex::decode(TEST_RECIPIENT_IN_SEPOLIA).unwrap();
    ...
    pub async fn sepolia_transfer_remote_bridged(&self, amount: u64) -> FixedBytes<32> {
        let recipient_address = hex::decode(TEST_RECIPIENT_IN_FUEL).unwrap();
        let parsed_address: FixedBytes<32> = FixedBytes::from_slice(recipient_address.as_slice());
